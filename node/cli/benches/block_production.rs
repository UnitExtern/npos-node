...
use clarus_node_cli as node_cli;
use clarus_runtime::{constants::currency::*, BalancesCall};
...
fn extrinsic_set_time(now: u64) -> OpaqueExtrinsic {
    clarus_runtime::UncheckedExtrinsic {
        signature: None,
        function: clarus_runtime::RuntimeCall::Timestamp(pallet_timestamp::Call::set { now }),
    }
    .into()
}
...
fn block_production(c: &mut Criterion) {
    ...
    group.bench_function(
        format!("{} transfers (no proof)", max_transfer_count),
        |b| {
            b.iter_batched(
                || extrinsics.clone(),
                |extrinsics| {
    ...
    group.bench_function(
        format!("{} transfers (with proof)", max_transfer_count),
        |b| {
            b.iter_batched(
                || extrinsics.clone(),
                |extrinsics| {
