...
use sp_crypto_hashing::blake2_256;use sp_inherents::InherentData;
use sp_runtime::{
...
impl TaskExecutor {
    fn new() -> Self {
        Self {
            pool: executor::ThreadPool::new().expect("Failed to create task executor"),
        }
    }
}
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let signed = self.keyring.sign(
            CheckedExtrinsic {
                signed: Some((
                    sender,
                    signed_extra(0, clarus_runtime::ExistentialDeposit::get() + 1),
                )),
                function: match self.content.block_type {
                    BlockType::RandomTransfersKeepAlive => {
                        RuntimeCall::Balances(BalancesCall::transfer_keep_alive {
                            dest: sp_runtime::MultiAddress::Id(receiver),
                            value: clarus_runtime::ExistentialDeposit::get() + 1,
                        })
                    }
                    BlockType::RandomTransfersReaping => {
                        RuntimeCall::Balances(BalancesCall::transfer_allow_death {
                            ...
                            value: 100 * DOLLARS - (clarus_runtime::ExistentialDeposit::get() - 1),
                        })
                    }
                    BlockType::Noop => {
